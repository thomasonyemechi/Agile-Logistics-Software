    ...
    <div class="container-fluid p-4">
        ...
        <div class="row">
                ...
                <div class="card mt-3">
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between pb-0 mb-0">
                            <div>
                                <h4 class="mb-0 fw-bold">{{ \App\Models\Delivery::where(['driver_id' => $driver->id, ])->sum('pieces') }}</h4>
                                <p class="fs-6 mb-0">Pallets assigned</p>
                            </div>
                            <div>
                                <h4 class="mb-0 fw-bold">{{ \App\Models\Delivery::where(['driver_id' => $driver->id, 'status' => 1 ])->sum('pieces') }}</h4>
            ...
            <div class="col-lg-8 col-md-12 col-12">
                @php
                    $deliveries = \App\Models\Delivery::where('driver_id', $driver->id)->orderBy('id', 'desc')->paginate(50);
                @endphp
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mb-0">Freight Deliveries</h4>
                    </div>
                    <div class="table-responsive">
                        <table class="table mb-0 text-nowrap">
                            <thead class="table-light">
                                <tr>
                                    <th scope="col" class="border-0">PO/PRO</th>
                            ...
                            <tbody>
                                @foreach ($deliveries as $del)
                                    <tr>
                                        <td class="align-middle"> {{$del->freight->bill_number}} </td>
                                        <td class="align-middle"> {{$del->pieces}}  {!! deliveryProStatus($del->status) !!}  </td>
                                        <td class="align-middle">{{ $del->freight->consignee }} <br> {{ $del->freight->consignee_phone }} {{ $del->freight->consignee_email }}  </td>
                                        <td class="align-middle"> {{$del->freight->destination}} </td>
                                        <td class="align-middle"> {{$del->freight->due_date}} </td>
